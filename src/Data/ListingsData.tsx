export const LISTING_DATA_SET = [
  {
    id: 1,
    title: "Let's play forza Horizon 5 and race marathons",
    url: '',
    desc: 'Join and lets race together in forza horizon. Want just to discus? Join anyway, every fun of forza is welcome here ;) P.S. We are newbies too, dont laught at us :D',
    gameName: 'Forza Horizon 5',
    gameImgUrl:
      'https://sm.ign.com/t/ign_pl/screenshot/default/5353_ypqg.1280.jpg',
  },
  {
    id: 2,
    title: 'Destiny 2 raids only',
    url: '',
    desc: 'Quisque suscipit ipsum eget convallis faucibus. In pellentesque pretium nisl, nec luctus arcu. Nunc fermentum tellus est, et gravida lacus finibus a. Nunc lacinia lorem et lectus.',
    gameName: 'Destiny 2',
    gameImgUrl: 'https://bigbaddice.pl/wp-content/uploads/2018/07/warmind2.jpg',
  },
  {
    id: 3,
    title: "Let's play forza Horizon",
    url: 'PayDay 2',
    desc: 'Nunc volutpat et eros a fringilla. Nullam quis finibus dui. Duis gravida, libero vitae hendrerit viverra, magna elit finibus massa, quis volutpat justo dui vitae dui.',
    gameName: 'PayDay 2',
    gameImgUrl:
      'https://cdn-ext.fanatical.com/production/product/1280x720/9f765b68-118f-432f-a282-c3e1cd1f976c.jpeg',
  },
  {
    id: 4,
    title: 'League of Legends rankeds plat',
    url: '',
    desc: 'Lorem ipsum dolor sit amet, nsequat vulputate quam. Phasellus lacus  nsequat nsequat vulputate quam. Phasellus vulputate quam. Phasellus libero.',
    gameName: 'League of Legends',
    gameImgUrl:
      'https://www.leagueoflegends.com/static/open-graph-2e582ae9fae8b0b396ca46ff21fd47a8.jpg',
  },
];
